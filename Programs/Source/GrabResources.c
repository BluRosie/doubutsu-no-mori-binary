...
switch(type)
      ...
      case 11:  /*npc_name_str*/
            if(!codeword(in,0x00E04000,&pbin,&ebin)) y=6;
            else if(!codeword(in,0x01BE6000,&pbin,&ebin)) y=8;
            pbin+=8;
            break;
      case 12:  /*20xxItems   */
...
if(!ptbl || !etbl)
  {etbl=ebin-pbin;
  if(type>11) y=10;  /*item lists are 10 bytes each*/
  for(x=y;x<=etbl;x+=y) {buf=byteswap(x);fwrite(&buf,4,1,tbl);}
  buf=0;
  fwrite(&buf,4,1,tbl);
  etbl=ftell(tbl);
  }
...
for(ptbl=0;ptbl<etbl;ptbl+=4)
   ...
   if(x>ebin) return 2;
   for(fseek(in,buf,SEEK_SET);buf<x;buf++) fputc(fgetc(in),bin);
   buf=x;
   }
