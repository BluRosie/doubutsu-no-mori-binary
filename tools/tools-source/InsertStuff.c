...
if(x==0x4641) return 0;
if(x==0x4146) return 1;   /*16bit byteswapping*/
fread(&x,2,1,in);
if(x==0x4146) return 2;   /*32bit byteswapping*/
return 3;
}
...
{int x=0;
 ...
 switch(code)
       {case 1: fprintf(stderr,"Can't find codeword for text - skipping"); break;
        ...
        case 5: case 6: fprintf(stderr,"\a\nThe rom is %sbit byteswapped.\nIt can't be byteswapped for this to work.\nStraighten it out and try again.",code>5 ? "32":"16"); x=1;break;
 ...
 fprintf(stderr,"\nPress ENTER to %s.", x==0 ? "continue":"quit"); getchar();
 if(x) exit(1);
return 1;}
...
{unsigned long start,end;
 if(codeword(in,0x010F4000,&start,&end)) return 0;
 start+=8;
 fseek(in,start,SEEK_SET);
 fread(&end,4,1,in);
 end=byteswap(end);
 if(end==0x02010000) return 1;
...
switch(type)
      {case 0:  /*message_data*/
            if(codeword(in,0x00CF9000,&ptbl,&etbl)) break;
            if(codeword(in,0x00BD4000,&pbin,&ebin)) codeword(in,0x01914000,&pbin,&ebin);
            break;
      case 1:   /*select_data */
            if(codeword(in,0x00D06000,&ptbl,&etbl)) codeword(in,0x00D09000,&ptbl,&etbl);
            if(codeword(in,0x00D05000,&pbin,&ebin)) codeword(in,0x01BA5000,&pbin,&ebin);
            break;
      case 2:   /*mail_data   */
            if(codeword(in,0x00D10000,&ptbl,&etbl)) break;
            if(codeword(in,0x00D07000,&pbin,&ebin)) codeword(in,0x01BA7000,&pbin,&ebin);
            break;
      case 3:   /*super_data  */
            if(codeword(in,0x00D12000,&ptbl,&etbl)) break;
            if(codeword(in,0x00D11000,&pbin,&ebin)) codeword(in,0x01BC7000,&pbin,&ebin);
            break;
      case 4:   /*ps_data     */
            if(codeword(in,0x00D15000,&ptbl,&etbl)) break;
            if(codeword(in,0x00D13000,&pbin,&ebin)) codeword(in,0x01BCA000,&pbin,&ebin);
            break;
      case 5:   /*string_data */
            if(codeword(in,0x00D18000,&ptbl,&etbl)) break;
            if(codeword(in,0x00D16000,&pbin,&ebin)) codeword(in,0x01BCE000,&pbin,&ebin);
            break;
      case 6:   /*maila_data  */
      ...
      case 11:  /*npc_name_str*/
            ...
            break;
      case 12:  /*20xxItems   */
            if(codeword(in,0x010F4000,&pbin,&ebin)) break;
            if(mods) {
	           ptbl=pbin+0x10;
		       etbl=ptbl+0x104;
		       ebin=pbin+0x1FA0;
...
if(pbin==0 || ebin==0) return 1;
if(!flag){
  if(ptbl==0 || etbl==0) return 2;
  x=etbl-ptbl;         /*x=size of table - final NULL entry*/
  if(x<tsize) {tsize=x; erratta(3);}
  }
...
if(!flag)
    ...
    if(ptbl==(etbl-4)) buf=0;
    fwrite(&buf,4,1,in);
    }
   if(buf==0) break;   

   buf=byteswap(buf);            /*start of next entry*/
   if(buf>bsize) {erratta(4); break;}
   if((buf+pbin)>ebin) {erratta(4); break;}
   
   fseek(bin,y,SEEK_SET);
   for(fseek(in,y+pbin,SEEK_SET);y<buf;y++) fputc(fgetc(bin),in);
   y=buf;
...
char list[29][15]={"\\message_data","\\select_data","\\mail_data","\\super_data","\\ps_data","\\string_data","\\maila_data","\\mailb_data","\\mailc_data","\\psz_data","\\superz_data","\\npc_name_str","\\20xxItems","\\21xxItems","\\22xxItems","\\23xxItems","\\24xxItems","\\25xxItems","\\26xxItems","\\27xxItems","\\28xxItems","\\29xxItems","\\2AxxItems","\\2BxxItems","\\2CxxItems","\\2DxxItems","\\2ExxItems","\\2FxxItems","\\1xxxItems"};
