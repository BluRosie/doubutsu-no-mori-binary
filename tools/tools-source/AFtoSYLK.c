    ...
    {if(argv[x][0]=='-' || argv[x][0]=='/')
      {switch(argv[x][1])
             {case 'Q': case 'q': errflag.Quiet=1;  break;
              ...
              case 'S': case 's': errflag.StrLength=0;    break;
...
if(errflag.LineNum) fprintf(out,"C;X1;Y%i;K\"%03X\"\n",yent,pos/4);
...
/*handle strings*/
   ...
   fprintf(out,"\"\n");
   if(errflag.StrLength) fprintf(out,"C;X%i;Y%i;K\"0x%X\"\n",2+(errflag.LineNum!=0),yent,next-org);
   if(!errflag.Quiet){
    if(errflag.NoteBad7F) {fprintf(out,"C;X%i;Y%i;K\"Command Exceeds 7F60\"\n",2+(errflag.LineNum!=0)+(errflag.StrLength!=0),yent);errflag.NoErrors=0;errflag.NoteBad7F=0;}
    if(errflag.NoteBad80) {fprintf(out,"C;X%i;Y%i;K\"Unknown 80XX command!\"\n",3+(errflag.LineNum!=0)+(errflag.StrLength!=0),yent);errflag.NoErrors=0;errflag.NoteBad80=0;}
    }
   yent++;
   cur=next;
   }/*end for()*/
