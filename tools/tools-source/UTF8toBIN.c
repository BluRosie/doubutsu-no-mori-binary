...
/*special cases to alias certain characters - they'll be noted below*/
    ...
    if(x==0x2A) return 0x85;           /*alias middle dot with asterix*/

/*returns are a special case*/
    if(x==0x5C) {if(y==0x52) return 0xCD;
    ...
    if(x==0x24)
      {switch(y)
             {case 'q': return 0x22;     /*$q quotation marks*/
              ...
              case 'n': case '8': return 0x2F;     /*$n eighth-note*/
...
token=strtok(buf," ]");
memset(vert,0,14);
/*do painfull comparisons to each string*/
...
/*7Fxx commands*/
  ...
  {vert[0]=0x7F; vert[1]=0;
  fwrite(vert,2,1,out);
  count=2;
  }
else if(!strcmp(token,"open"))
  {vert[0]=0x7F; vert[1]=1;
  fwrite(vert,2,1,out);
  count=2;
  }
else if(!strcmp(token,"CLS"))
  {vert[0]=0x7F; vert[1]=2;
  fwrite(vert,2,1,out);
  count=2;
  }
else if(!strcmp(token,"pause"))
  {if(!(token=strtok(NULL," ]"))) {erratta(3,0); x=0;}
  else x=strtol(token,NULL,10);
  vert[0]=0x7F; vert[1]=3; vert[2]=x%256;
  fwrite(vert,3,1,out);
  count=3;
  }
else if(!strcmp(token,"wait"))
  {vert[0]=0x7F; vert[1]=4;
  fwrite(vert,2,1,out);
  count=2;
  }
  ...
  if(token=strtok(NULL," ]")){         /*green*/
    x=strtol(token,NULL,10);
    vert[3]=x%256;}

  if(token=strtok(NULL," ]")){         /*blue*/
    x=strtol(token,NULL,10);
    vert[4]=x%256;}
  else erratta(5,0);
  ...
  {if(!(token=strtok(NULL," ]"))) {erratta(6,0); x=0;}
  if(!strcmp(token,"off")) vert[1]=7;
  else vert[1]=6;
  vert[0]=0x7F;
  fwrite(vert,2,1,out);
  count=2;
...
/*one, two, skip a few*/
  ...
  {if(!(token=strtok(NULL," ]"))) {erratta(14,0); x=0;}
  x=strtol(token,NULL,16);
  x=x%0x10000;
  vert[0]=0x7F; vert[1]=0x0E;
  ...
  if(token=strtok(NULL," ]")) {
    x=strtol(token,NULL,10);
    if(x>6 || x<1) {erratta(15,0); x=1;}
    if(x>4) x+=0x64;        /*catch AC commands*/
    vert[1]=vert[1]+x;
    }
  if(token=strtok(NULL," ]")) {
    x=strtol(token,NULL,16);
...
/*these are the 3 list types
  ...
  if(token=strtok(NULL," ]")){
    x=strtol(token,NULL,16);             /*option 1*/
    x=x%0x10000;
    vert[count]=x/0x100;
    vert[count+1]=x%0x100; count+=2;}
  ...
  if(token=strtok(NULL," ]")){         /*option 2*/
    ...
    vert[count]=x/0x100;
    vert[count+1]=x%0x100; count+=2;}
  else {if(!strcmp(token,"menu")) erratta(16,0);
       else erratta(13,0);
       count=6;}
  ...
  if(vert[1]==0x18){
     ...
     x=x%0x10000;
     vert[count]=x/0x100;
     vert[count+1]=x%0x100; count+=2; vert[1]=0x79;}
  }
